...
const spawn = ({ spawnerName, role, maxTypeOfRole, roleParts, energySource }) => {
    ...
    const existingUnitsOfRole = _.filter(creepsInRoom, (creep) => creep.memory.role == role);
    console.log(role + 's: ' + existingUnitsOfRole.length);
    
    if (existingUnitsOfRole.length < maxTypeOfRole) {
        const newName = role + Game.time;
        console.log('Spawning new ' + role + ': ' + newName);
        const spawnResult = Game.spawns[spawnerName].spawnCreep(roleParts, newName, {
            memory: {
                role,
                energySource
            }
        ...
        } else throw Error('Unable to spawn a new ' + role + ' right now, but I would have!');
    }
}
